...
$parbaude     = $parbaudes[$inspection['protocol_number']];
$lifts        = $lifti[ $parbaude['parbaude_lifts_reg_nr'] ];
$valditajs_id = $parbaude['parbaude_valditajs'];
$valditajs    = $valditaji[ $valditajs_id ];
// var_dump( $valditajs );
// die();
...
$lifts_adrese_novads      = $lifts['lifts_adrese_novads'];
...
$valditajs_kontakt_person          = $valditajs['valditajs_kontakt_person'];
$valditajs_kontakt_person_amats    = $valditajs['valditajs_kontakt_person_amats'];
...
// ======================================================================================================
if ($valditajs_id === '4' ) {
    $pdf->cell($parbaude_piedalijas_1col_width + $parbaude_piedalijas_2col_width, 4.5, $valditajs_nosaukums, 0, 0, 'L', false);
    ...
    $pdf->cell($parbaude_piedalijas_1col_width + $parbaude_piedalijas_2col_width, 4.5, 'Ar pārbaudes rezultātiem iepazinos: ' . $valditajs_nosaukums, 0, 0, 'L', false);
    $pdf->cell($parbaude_piedalijas_4col_width, 5, 'Pārbaudes datums', 0, 0, 'L', false);
    $pdf->cell($parbaude_piedalijas_5col_width + 5, 5, $parbaude_datums, 'B', 1, 'C', false);
} else {
...
if ($valditajs_id === '4' || $valditajs_id === '1' || $valditajs_id === '2' ) {
    $pdf->cell($parbaude_piedalijas_1col_width - 30, 5, $valditajs_kontakt_person_amats . '     ' . $valditajs_kontakt_person, '', 0, 'L', false);
}
...
if (( ! $is_atkartota && ! $is_arpuskartas && $valditajs_protokols_ar_merijumiem ) ) {
    ...
    foreach ( $checkboxes as $checkbox ) {
        $pdf->Image(__DIR__.'/../img/checkbox.png', $checkbox['x'] + $el_merijumi_second_table_col_data_width / 2 - $checkbox_image_width / 2, $checkbox['y'], $checkbox_image_width, $checkbox_image_height, );
    }
    foreach ( $empty_checkboxes as $empty_checkbox ) {
        $pdf->Image(__DIR__.'/../img/empty_checkbox.png', $empty_checkbox['x'] + $el_merijumi_second_table_col_data_width / 2 - $checkbox_image_width / 2, $empty_checkbox['y'], $checkbox_image_width, $checkbox_image_height, );
    }
    // var_dump($checkboxes);
