...
import { PDFViewer, Page, Text, View, Document, StyleSheet, Font, Image, Svg, PDFDownloadLink } from '@react-pdf/renderer';
                ...
                {inspectionsNewestFirst[0].whatsWrong1.map((nonComp) => (
                    <Text
                        style={{
                            ...sharedStyles.text,
                            maxWidth: '135mm',
                            fontSize: neatbilstibuSkaits >= 10 ? '3mm' : '3.5mm',
                        }}
                    >
                        {nonComp}
                    </Text>
...
export default function ({ lift }: { lift: LiftWithInspections }) {
    ...
    const inspectionsNewestFirst = inspections
        .sort((insp1, insp2) => dayjs(insp2.date_start) - dayjs(insp1.date_start))
        .map((inspection) => {
            return {
